...
namespace MatrixGame
{
    class MatrixGame
    {
        ...
        static int checkInfo(double score, double badSeries, int rounds, int series)
        {
            string userString = Console.ReadLine();
            while (userString == "score")
            {
                yourScore(score, badSeries, rounds, series);
        ...
        static double[] generateMatrixHelp(int rows, int cols, double yourPoints, double badSeries, int rounds, int series)
        {
            Random rand = new Random();
            int[,] matrix = new int[rows, cols];
            int guessingRow = rand.Next(0, rows);
            int guessingCol = rand.Next(0, cols);
            double[] information = new double[2];  //this will contain player information and...
            ...
            int userRow = userRowCheck(rows, yourPoints, badSeries, rounds, series);

            Console.WriteLine("Col: ");
            int userCol = userCowCheck(cols, yourPoints, badSeries, rounds, series);
            ...
            for (int r = 0; r < rows; r++)
            {
                for (int c = 0; c < cols; c++)
                {
                    ...
                    else
                    {
                        ...
                        doYouWantHelp(wrongCounter, guessingRow, guessingCol, yourPoints, badSeries, rounds, series);
                        ...
                        {
                            Console.Write("Row: ");
                            userRow = userRowCheck(rows, yourPoints, badSeries, rounds, series);
                            Console.Write("Cow: ");     
                            userCol = userCowCheck(cols, yourPoints, badSeries, rounds, series);
                            ...
                            Console.WriteLine("Не познахте. Моля опитайте отново.");
                            badSeries++;
                            information[1] = badSeries;
                            if (badSeries > 2)
                            {
                                min = rand.Next(0, 5);
                                max = rand.Next(5, 20);
                                punishment = min + rand.NextDouble() * (min + max);
                                if (punishment < yourPoints)
                                {
                                    yourPoints -= punishment;
                                }
                                else
                                {
                                    punishment = rand.Next(0, (int)yourPoints);
                                    yourPoints -= punishment;
                                }
                                Console.WriteLine("Правите серийка от {0} грешни отговора :(", badSeries);
                                Console.WriteLine("Заради лошата серийка получавате наказание. Губите {0:f1} точки",      punishment);
                            }
                            Console.ForegroundColor = ConsoleColor.Gray;
                            ...
                            if (wrongCounter > 3)
                            {
                                ...
                                Console.WriteLine("Do you want more help ? Yes : No");
                                string answer = checkYesNo(yourPoints, badSeries, rounds, series);
                                if (answer == "yes")
                                {
                                    if (userRow == guessingRow && userCol == guessingCol)
                                    {
        ...
        static double[] generateMatrix(int rows, int cols, double yourPoints, double badSeries, int rounds, int series)
        {
            Random rand = new Random();
            int[,] matrix = new int[rows, cols];
            int guessingRow = rand.Next(0, rows);
            int guessingCol = rand.Next(0, cols);
            double[] information = new double[2];  //this will contain player information and...
                                                   //first element of array return players score, second one                                negative series
        ...
        static int userRowCheck(int rows, double score, double badSeries, int rounds, int series)
        {
        ...
        static int userCowCheck(int cows, double score, double badSeries, int rounds, int series)
        {
        ...
        static void doYouWantHelp(int wrongNumbers, int RightRows, int RightCols, double score, double badSeries, int rounds, int series)
        {
            Random rand = new Random();
            if (wrongNumbers > 1)
            {
                ...
                string wantHelp = checkYesNo(score, badSeries, rounds, series);
        ...
        static string checkYesNo(double score, double badSeries, int rounds, int series)
        {
            Random rand = new Random();
        ...
        static void yourScore(double score, double badSeries, int rounds, int series)
        {
            Console.ForegroundColor = ConsoleColor.Green;
            ...
            if (series > 0)
            {
                ...
                Console.WriteLine("In the end of game you will receive additional points for the good work.");
                Console.ForegroundColor = ConsoleColor.Green;
            }
            ...
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine("Your bad series {0}", badSeries);
                Console.ForegroundColor = ConsoleColor.Green;
            }
        ...
        //метод, който да пресмята бонус точките при правилните серийки
        static double bonus(int series)
        {
            Random rand = new Random();
            ...
            if (series > 0)
            {
                bonusPoints = (min + rand.NextDouble() * (max - min)) * series;
            }
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                string autoGenerate = checkYesNo(yourPoints, badSeries, rounds, series);
                ...
                if (autoGenerate == "yes")
                {
                    ...
                    string help = checkYesNo(yourPoints, badSeries, rounds, series);

                    if (help == "yes")
                    {
                        series = calcPositiveSeries(rounds, series);
                        information = generateMatrixHelp(matrixInfo.matrixRows, matrixInfo.matrixCols, yourPoints, badSeries, rounds, series);
                        yourPoints += information[0];
                        recivedBonus = bonus(series);
                    ...
                    else
                    {
                        ...
                        information = generateMatrix(matrixInfo.matrixRows, matrixInfo.matrixCols, yourPoints, badSeries, rounds, series);
                        yourPoints += information[0];
                        recivedBonus = bonus(series);
                ...
                else
                {
                    ...
                    int rows = checkInfo(yourPoints, badSeries, rounds, series);


                    Console.WriteLine("Enter matrix[cols]: ");
                    //int cols = int.Parse(Console.ReadLine());
                    int cols = checkInfo(yourPoints, badSeries, rounds, series);
                ...
                badSeries += information[1];


                //Това най-отдолу
                Console.WriteLine("Do you want one more game: yes or no");
                //string command = Console.ReadLine().ToLower();
                string command = checkYesNo(yourPoints, badSeries, rounds, series);
                if (command == "no")
                {
                    Console.ForegroundColor = ConsoleColor.Green;
                    Console.WriteLine("Your points {0:f1}", yourPoints);
                    ...
                    {
                        Console.ForegroundColor = ConsoleColor.Red;
                        Console.WriteLine("Negative series: {0}", information[1]);
                    }
                   
                    if (recivedBonus > 0)
                    {
                        yourPoints += recivedBonus;
                        Console.ForegroundColor = ConsoleColor.Green;
                        Console.WriteLine("You receive bonus {0:f1} points", recivedBonus);
                        Console.ForegroundColor = ConsoleColor.Green;
                        Console.WriteLine("Your points with bonus {0:f1}", yourPoints);
